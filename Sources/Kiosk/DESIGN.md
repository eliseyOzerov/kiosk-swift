...
The core runtime type is `HttpContext`. It carries the base URL, session, headers, content-type defaults, accepted response type, wire settings, error decoding, request options, and wrappers. Route macros copy and refine that context as the call path moves from the root to a specific endpoint.
